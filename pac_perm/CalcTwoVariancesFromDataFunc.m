...
function [N_VEC N_HALF_VEC Z_sig_mean Q_sig_mean ] ...
    = CalcTwoVariancesFromDataFunc(R, alpha_vec, rand_flag, iters, sampling_flag )
...
% % % % % % % % %         end
        ...
        if(update_i == 1)
            ...
            for j=1:chunks_num
                ...
                chunk_normed_data = chunk_normed_data ./ max(repmat(sqrt(sum(chunk_normed_data.^2, 2)),1, N), TOL); % Avoid division by zero
...
%                chunk_Fisher_Zs(j,:) = 0.5 * (log(1+chunk_Pearson_corrs) - log(1-chunk_Pearson_corrs));  % Fisher-Z-transform
    ...
    if(chunks_num > 1)
        chunk_Z_sig = (chunk_Z_sig./iters); % Now move from variance to std.
