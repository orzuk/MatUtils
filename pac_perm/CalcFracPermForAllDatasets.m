% Here witergase load a data, computsec the correlations and geOt the
% desired
% fraction
...
% Choose which data to load - (can run on all datasets)

num_samples_needed = zeros(1,2);
std_needed = zeros(1,2);

data_output_strct = {}; % Structure keeping the whole data's vars

load('all_datas_vars.mat');

data_output_strct{30} = 'dummy'; % Make the struct big enough to include empty dummy cells.

SMALL_GENES_NUM = 1000; % Take only this amount of genes ...

% Do only part of the work: From 9 to 14
for data_flag =  [NEW_ROSSETA WANG ROSEN HEPATO Bhattacharjee BEER] %  OLD_VANT_VEER % Bhattacharjee %%% [YEOH Bhattacharjee ROSEN]% RAND_DATA % OLD_VANT_VEER% OLD_VANT_VEER % [RAND_DATA] % WANG %OLD_VANT_VEER % HEPATO % YEOH % HEPATO  % NEW_ROSSETA; % BRAIN; % BRAIN; % HEPATO % OLD_VANT_VEER % HEPATO % TOPIC % OLD_VANT_VEER % NEW_ROSSETA % NIPS_GISETTE % NEW_ROSSETA % NIPS_GISETTE; %NIPS_DEXTER; %%NIPS_ARCENE; %NIPS_DOROTHEA; % TOPIC; % WANG; %%% NEW_ROSSETA; %%%OLD_VANT_VEER;
...
% My pre-processing        
        ...
        % Take correlations the same as VantVeer Old
        Q_corr_vec = 0.0166*(randn(1, Ngenes)); % Correlation of each gene with the survival
        W_corr_vec = 0.0166*(randn(1, Ngenes));
    ...
    if(  (estimate_variance_flag == TRUE))  %%% (calc_corrs_flag == TRUE) &  % For now we do this always - not demand that we calculate corrs
        ...
        if( (isempty(data_output_strct{data_flag})) || (corrs_iters2 > data_output_strct{data_flag}.ITERS_USED)) % Do only stuff that was not done before
            % Skip the following: very time-consuming
            [N_VEC2 N_HALF_VEC2 BS_Z_sig_mean_plot BS_Q_sig_mean_plot ] = ...
                CalcTwoVariancesFromDataFunc(R, alpha, rand_flag, corrs_iters2, BOOTSTRAP );

            [N_VEC2 N_HALF_VEC2 Z_sig_mean_plot Q_sig_mean_plot ] = ...
                CalcTwoVariancesFromDataFunc(R, alpha, rand_flag, corrs_iters2, NON_OVERLAP );
        else % instead of calculating the variances we just read them from the file
            N_HALF_VEC = data_output_strct{data_flag}.N_HALF_VEC;   N_HALF_VEC2=N_HALF_VEC;
            N_VEC = data_output_strct{data_flag}.N_VEC;             N_VEC2=N_VEC;
            Z_sig_mean_plot =  data_output_strct{data_flag}.Z_STD;
            Q_sig_mean_plot =  data_output_strct{data_flag}.Q_STD;
            BS_Q_sig_mean_plot = data_output_strct{data_flag}.BS_Q_STD;
            BS_Z_sig_mean_plot = data_output_strct{data_flag}.BS_Z_STD;
            corrs_iters2=data_output_strct{data_flag}.ITERS_USED;
        end % calculating the variances

        % Do a fit to the variances we've got:this is done Always!
        % We assume that Z_std = a*n^b and Q_str = m+c*n^d

        % Now do fit : NEW! Everything is fitted to n-3!
        linfit = fittype('b*x+a');
        Z_fit = fit(log(N_HALF_VEC2'-3), log(Z_sig_mean_plot'), linfit);
        a = Z_fit.a; b = Z_fit.b;
        % Note: This gives different (and crappy) results !
        orig_fit = fittype('a*x^b');
        ZZ_fit = fit(N_HALF_VEC2'-3, Z_sig_mean_plot', orig_fit);

        Z_fit = fit(N_HALF_VEC2'-3, Z_sig_mean_plot', 'power1');
        Q_fit = fit(N_VEC2'-3, Q_sig_mean_plot', 'power2');

        % Do fit also for the variance
        Var_Z_fit=fit(N_HALF_VEC2'-3, Z_sig_mean_plot'.^2, 'power1');
        Var_Q_fit = fit(N_VEC2'-3, Q_sig_mean_plot'.^2, 'power2');

        % Plot and compare both variances
        figure; subplot(1,2,1); hold on; plot(N_HALF_VEC2, Z_sig_mean_plot, '.'); plot(N_VEC2, Q_sig_mean_plot, '.r');
        plot(N_HALF_VEC2, Q_sig_mean_plot(1:length(N_HALF_VEC2))-Z_sig_mean_plot, '.g');
        plot(N_HALF_VEC2, 1 ./ sqrt(N_HALF_VEC2-3), 'k');    plot(N_VEC2, std(Fisher_Zs) + 1 ./ sqrt(N_VEC2-3), 'k');
        plot(N_HALF_VEC2,  ZZ_fit.a .* N_HALF_VEC2.^ZZ_fit.b, 'm');
        plot(N_VEC2, Q_fit.c + Q_fit.a .* N_VEC2 .^ Q_fit.b, 'c');
        legend('Ave Z std.', 'Q std.', 'diff', 'Fisher', 'Fisher plus one', 'Z fit', 'Q fit'); xlabel('Nsamples'); ylabel('Std');
        title(['Std. of each genes Z and of total Q, Data is ' data_str]);

        % Plot and compare both variances on LogLog scale
        subplot(1,2,2); hold on; plot(log(N_HALF_VEC2), log(Z_sig_mean_plot), '.');
        plot(log(N_VEC2), log(Q_sig_mean_plot), '.r');
        plot(log(N_HALF_VEC2), Z_fit.b .* log(Z_fit.b.*N_HALF_VEC2), 'm');
        ...
        data_output_strct{data_flag}.DATA_NAME = data_str;
        data_output_strct{data_flag}.ITERS_USED = corrs_iters2; % record how many iterations we have used in order to produce the fit
        data_output_strct{data_flag}.N_VEC = N_VEC2;
        data_output_strct{data_flag}.N_HALF_VEC = N_HALF_VEC2;
        data_output_strct{data_flag}.Q_STD = Q_sig_mean_plot;
        data_output_strct{data_flag}.Z_STD = Z_sig_mean_plot;

        % update the fitting coefficients for Q and Z
        data_output_strct{data_flag}.A = Q_fit.a;
        data_output_strct{data_flag}.B = Q_fit.b;
        data_output_strct{data_flag}.C = Q_fit.c;
        data_output_strct{data_flag}.Z_A = Z_fit.a;
        data_output_strct{data_flag}.Z_B = Z_fit.b;

        data_output_strct{data_flag}.Var_A = Var_Q_fit.a;
        data_output_strct{data_flag}.Var_B = Var_Q_fit.b;
        data_output_strct{data_flag}.Var_C = Var_Q_fit.c;
        data_output_strct{data_flag}.Var_Z_A = Var_Z_fit.a;
        data_output_strct{data_flag}.Var_Z_B = Var_Z_fit.b;




        % Now save everything from the bootstrap
        data_output_strct{data_flag}.BS_Q_STD = BS_Q_sig_mean_plot;
        data_output_strct{data_flag}.BS_Z_STD = BS_Z_sig_mean_plot;

        BS_Z_fit = fit(N_HALF_VEC2'-3, BS_Z_sig_mean_plot', 'power1');
        BS_Q_fit = fit(N_VEC2'-3, BS_Q_sig_mean_plot', 'power2');
        % Do fit also for the variance
        BS_Var_Z_fit=fit(N_HALF_VEC2'-3, Z_sig_mean_plot'.^2, 'power1');
        BS_Var_Q_fit = fit(N_VEC2'-3, Q_sig_mean_plot'.^2, 'power2');

        data_output_strct{data_flag}.BS_A = BS_Q_fit.a;
        data_output_strct{data_flag}.BS_B = BS_Q_fit.b;
        data_output_strct{data_flag}.BS_C = BS_Q_fit.c;
        data_output_strct{data_flag}.BS_Z_A = BS_Z_fit.a;
        data_output_strct{data_flag}.BS_Z_B = BS_Z_fit.b;
        data_output_strct{data_flag}.BS_Var_A = BS_Var_Q_fit.a;
        data_output_strct{data_flag}.BS_Var_B = BS_Var_Q_fit.b;
        data_output_strct{data_flag}.BS_Var_C = BS_Var_Q_fit.c;
        data_output_strct{data_flag}.BS_Var_Z_A = BS_Var_Z_fit.a;
        data_output_strct{data_flag}.BS_Var_Z_B = BS_Var_Z_fit.b;

        % Save immediately to be on the safe side
        save all_datas_vars.mat data_output_strct
    ...
    % % % % %     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % % % % %     % Many figures. Disable for now:
    % % % % %     figure; imagesc(Orig_CorrMatrix(sort_inds,sort_inds)); colorbar; title('Correlations in Expression');
    % % % % %     figure; imagesc(Var_Z_CorrMatrix(sort_inds(1:end-1),sort_inds(1:end-1))); colorbar;
    % % % % %     title('Correlations in noise of correlation with survival');
    ...
    for iters = iters_vec
        now_iter_is = iters
        for true_corr_flag=TWO_SAMPLED % Go over the two options   %%% TRUE_AND_SAMPLED = 1; TWO_SAMPLED = 0;
            ...
            inf_limit_Z_var_frac = zeros(2 * length(alpha_vec), length(nsamples_vec));
            inf_limit_Z_var_std = zeros(2 * length(alpha_vec), length(nsamples_vec));
            inf_limit_Fisher_var_frac = zeros(2 * length(alpha_vec), length(nsamples_vec));
            inf_limit_Fisher_var_std = zeros(2 * length(alpha_vec), length(nsamples_vec));
            ...
            Fisher_Zs_std_Fisher_var = sqrt(Fisher_Zs_std^2 - 1/(Nsamples-3));

            % Set the std. based on each gene's variance
            Fisher_Zs_std_Z_var = sqrt(Fisher_Zs_std^2 - (data_output_strct{data_flag}.Z_A .* Nsamples ^ data_output_strct{data_flag}.Z_B)^2);


            % Set the std. as the one given by fit to infinity
            Fisher_Zs_std = sqrt(data_output_strct{data_flag}.Var_C); % New!!! Take the Var !!!
%            Fisher_Zs_std = 0.0166; % Try taking it from the data !!!! 
%            Fisher_Zs_std = std(Fisher_Zs); % Try to take straight from the data. No corrections !!! 
            
            
            % Make sure everybody is real (just to not throw the program)
            Fisher_Zs_std_Z_var=abs(Fisher_Zs_std_Z_var); Fisher_Zs_std=abs(Fisher_Zs_std);
            
            % Calculate the 'true' Fisher Zs by rescaling
            Fisher_Zs_true = (Fisher_Zs ./ std(Fisher_Zs)) .* sqrt(data_output_strct{data_flag}.Var_C);
            Fisher_Zs_true = Fisher_Zs_true';
            
            for(nsamples=nsamples_vec)

                done_nsamples = nsamples

                % Adjust alpha to have N_TOP=n
                if(alpha_type==LINEAR_ALPHA)
                    alpha= nsamples/Ngenes;
                end

                % Compute effective nsamples based on struct from fit to infinity
                nsam_eff = data_output_strct{data_flag}.Var_A * (nsamples-3)^(data_output_strct{data_flag}.Var_B) ; % + data_output_strct{data_flag}.C Do not add!!!!
                nsam_eff = 1/nsam_eff; % Transfer from variance to # samples
                ...
                [inf_limit_frac(index, i) inf_limit_std(index, i) x_alpha ] = ...
                    compute_inf_limit_fraction(rand_flag, one_side_flag, true_corr_flag, Fisher_Zs_std , ...
                    nsam_eff, alpha,miu,prior);  % We use here the approximation of Fisher
                % In the above we're cheating. The '1' should be replaced
                % by Fisher_Zs_std

                nsam_eff_Z_var = data_output_strct{data_flag}.Z_A .* nsamples ^ data_output_strct{data_flag}.Z_B;
                nsam_eff_Z_var = 1/(nsam_eff_Z_var^2);

                % Here do the analytic calculations, where taking the noise on each gene
                [inf_limit_Z_var_frac(index, i) inf_limit_Z_var_std(index, i) x_alpha ] = ...
                    compute_inf_limit_fraction(rand_flag, one_side_flag, true_corr_flag, Fisher_Zs_std_Z_var , ... % Fisher_Zs_std_Z_var , ...
                    nsam_eff_Z_var, alpha,miu,prior);  % We use here the approximation of Fisher % nsam_eff_Z_var*2

              %%%  [inf_limit_Fisher_var_frac(index, i) inf_limit_Fisher_var_std(index, i) x_alpha ] = ...
              %%%      compute_inf_limit_fraction(rand_flag, one_side_flag, true_corr_flag, Fisher_Zs_std_Fisher_var , ... % Fisher_Zs_std_Fisher_var , ...
              %%%      nsamples-3, alpha,miu,prior);  % We use here the approximation of Fisher % (nsamples-3)*3
                ...
                [kept_frac_dist f_all_genes_from_data f_all_genes_from_data_one_NTOP] = ...
                    sample_kept_fraction_distribution(rand_flag, one_side_flag, ...
                    true_corr_flag, Fisher_Zs_true, Fisher_Zs_std, nsam_eff, Ngenes, alpha, iters, R, ...
                    FALSE, miu, prior, 0.000000001 * Z_sig_std/Z_sig_mean, coeffs_vec, corr_mat, sampling_flag);
                ...
                if((true_corr_flag == TWO_SAMPLED) & (nsamples<=Nsamples/2)) % For now skip this !!!!
                    ...
                    [ kept_frac_dist f_all_genes_from_data f_all_genes_from_data_one_NTOP]  = ...
                        sample_kept_fraction_distribution(rand_flag, one_side_flag, ...
                        true_corr_flag, Fisher_Zs_true, Fisher_Zs_std, nsamples, Ngenes, alpha, iters, R, ...
                        TRUE, miu , prior,  0.000000001 * Z_sig_std/Z_sig_mean, coeffs_vec, corr_mat, sampling_flag);
            ...
            inf_limit_Z_var_std(index,:) = inf_limit_Z_var_std(index,:) ./ sqrt(Ngenes);
            inf_limit_Fisher_var_std(index,:) = inf_limit_Fisher_var_std(index,:) ./ sqrt(Ngenes);
            ...
            if(true_corr_flag == TRUE_AND_SAMPLED)
                ...
                errorbar(nsamples_vec(1:small_vec_len), samp_frac_mean_from_data(index,1:small_vec_len), ...
                    samp_frac_std_from_data(index,1:small_vec_len), 'm');
                ...
                %                plot(nsamples_vec, inf_limit_Z_var_frac(index,:), 'c');
                errorbar(nsamples_vec, inf_limit_Z_var_frac(index,:), inf_limit_Z_var_std(index,:), 'c');

                % plot(nsamples_vec, inf_limit_Fisher_var_frac(index,:), 'k');
%%%%                errorbar(nsamples_vec, inf_limit_Fisher_var_frac(index,:), inf_limit_Fisher_var_std(index,:), 'y');
                ...
                legend( 'ind. std.', 'ind. std.',  'sampled from model','sampled from data','sampled from data',...
                    'Saddle approx.', 'Saddle approx. Z',  '\alpha (RAND)',  2);  % just before last, add :  'sampled from data',
                % % %             legend('sampled from model', 'Saddle approx.', 2);
            end
            ...
            data_output_strct{data_flag}.f_iters = iters; % How many iterations did we use?
            data_output_strct{data_flag}.N_f_vec = nsamples_vec;
            data_output_strct{data_flag}.N_f_small_vec = nsamples_vec(1:small_vec_len);
            data_output_strct{data_flag}.f_vec_saddle_mean = inf_limit_frac(index,:);
            data_output_strct{data_flag}.f_vec_saddle_std = inf_limit_std(index,:);
            data_output_strct{data_flag}.f_vec_sim_mean = samp_frac_mean(index,:);
            data_output_strct{data_flag}.f_vec_sim_std = samp_frac_std(index,:);
            data_output_strct{data_flag}.f_vec_model_sim_mean = samp_frac_mean_from_data(index,1:small_vec_len);
            data_output_strct{data_flag}.f_vec_model_sim_std = samp_frac_std_from_data(index,1:small_vec_len);
            data_output_strct{data_flag}.alpha = alpha;
        ...
        now_doing_the_gaussian_fit_plot = 999999
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Figure 6 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
