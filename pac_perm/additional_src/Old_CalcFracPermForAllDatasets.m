% Here witergase load a data, computsec the correlations and geOt the desired
% fraction
...
BOOTSTRAP=0; NON_OVERLAP=1; 
sampling_flag = BOOTSTRAP;

global CONST_ALPHA LINEAR_ALPHA;
CONST_ALPHA = 0; LINEAR_ALPHA=1;
alpha_type = LINEAR_ALPHA; % Choose whether alpha is kept constant or alpha*N_g ~ n
...
for data_flag =  1:14 % [RAND_DATA] % WANG %OLD_VANT_VEER % HEPATO % YEOH % HEPATO  % NEW_ROSSETA; % BRAIN; % BRAIN; % HEPATO % OLD_VANT_VEER % HEPATO % TOPIC % OLD_VANT_VEER % NEW_ROSSETA % NIPS_GISETTE % NEW_ROSSETA % NIPS_GISETTE; %NIPS_DEXTER; %%NIPS_ARCENE; %NIPS_DOROTHEA; % TOPIC; % WANG; %%% NEW_ROSSETA; %%%OLD_VANT_VEER;
    ...
    if(  (estimate_variance_flag == TRUE))  %%% (calc_corrs_flag == TRUE) &  % For now we do this always - not demand that we calculate corrs
        ...
        if( (isempty(data_output_strct{data_flag})) || (corrs_iters2 > data_output_strct{data_flag}.ITERS_USED)) % Do only stuff that was not done before
            ...
            [N_VEC2 N_HALF_VEC2 BS_Z_sig_mean_plot BS_Q_sig_mean_plot ] = ...
                CalcTwoZVariancesFromDataFunc(R, alpha, rand_flag, corrs_iters2, BOOTSTRAP );
            
            [N_VEC2 N_HALF_VEC2 Z_sig_mean_plot Q_sig_mean_plot ] = ...
                CalcTwoZVariancesFromDataFunc(R, alpha, rand_flag, corrs_iters2, NON_OVERLAP );
                         
            % Do a fit to the variances we've got:
            ...
            Z_fit = fit(N_HALF_VEC2', Z_sig_mean_plot', 'power1');
            Q_fit = fit(N_VEC2', Q_sig_mean_plot', 'power2');

            % Do fit also for the variance
            Var_Z_fit=fit(N_HALF_VEC2', Z_sig_mean_plot'.^2, 'power1');
            Var_Q_fit = fit(N_VEC2', Q_sig_mean_plot'.^2, 'power2');
            ...
            BS_Z_fit = fit(N_HALF_VEC2', BS_Z_sig_mean_plot', 'power1');
            BS_Q_fit = fit(N_VEC2', BS_Q_sig_mean_plot', 'power2');
            % Do fit also for the variance
            BS_Var_Z_fit=fit(N_HALF_VEC2', Z_sig_mean_plot'.^2, 'power1');
            BS_Var_Q_fit = fit(N_VEC2', Q_sig_mean_plot'.^2, 'power2');          
...
% % % % %     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for iters = iters_vec
        ...
        for true_corr_flag=TWO_SAMPLED % Go over the two options   %%% TRUE_AND_SAMPLED = 1; TWO_SAMPLED = 0;
            ...
            Fisher_Zs_std = data_output_strct{data_flag}.C;
            ...
            for(nsamples=nsamples_vec)
                ...
                nsam_eff = data_output_strct{data_flag}.A * nsamples^(data_output_strct{data_flag}.B) ; % + data_output_strct{data_flag}.C Do not add!!!! 
                nsam_eff = 1/(nsam_eff^2);
                ...
                [kept_frac_dist f_all_genes_from_data f_all_genes_from_data_one_NTOP] = sample_kept_fraction_distribution(rand_flag, one_side_flag, ...
                    true_corr_flag, Fisher_Zs_std, nsam_eff, Ngenes, alpha, iters, R, ...
                ...
                if((true_corr_flag == TWO_SAMPLED) & (nsamples<=Nsamples/2)) % For now skip this !!!!
                    ...
                    [ kept_frac_dist f_all_genes_from_data f_all_genes_from_data_one_NTOP]  = sample_kept_fraction_distribution(rand_flag, one_side_flag, ...
                        true_corr_flag, Fisher_Zs_std, nsamples, Ngenes, alpha, iters, R, ...
...
%                plot(nsamples_vec, inf_limit_Z_var_frac(index,:), 'c');
                ...
                legend( 'ind. std.', 'ind. std.',  'sampled from model','sampled from data','sampled from data',...
                    'Saddle approx.', 'Saddle approx. Z', 'saddle approx Fisher', '\alpha (RAND)',  2);  % just before last, add :  'sampled from data',
                % % %             legend('sampled from model', 'Saddle approx.', 2);
            end
