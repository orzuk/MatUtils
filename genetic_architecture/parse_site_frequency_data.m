...
% R = loadcellfile(site_frequency_file_name);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%% Stage 2: Extract All Info Fields to Population %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
compute_snp_info=1
...
if(extract_fields_flag)
    for population = populations_vec  % save different files per population
        ...
        for j=1:length(S.field_names) % loop on all fields
            prepare_field=j
        ...
        for i=1:num_snps
            ...
            S.XXX_FEATURE_{i} = str2word(',', tmp_str{19}(4:end), 1);
            S.GENE{i} = tmp_str{11}(4:end); % get gene name
            for j=1:length(S.field_names) % loop on all fields
                field_ind = strmatch([S.field_names{j} '='], tmp_str);
                eval_str = ['S.' S.field_descriptions{j} '{' num2str(i) '} = ''' ...
                    str2word('=', tmp_str{field_ind}, 2) ''';'];
                eval(eval_str);
            end
        end % loop on SNPs
        S = my_rmfield(S, {'European', 'African', 'dbSNP', 'Total', 'Minor', ...
            'Observed', 'Average', 'geneList', 'Whether', 'PubMed', 'GenotypeMat'});
        if(isfield(S, 'scorePhastCons') && iscell(S.scorePhastCons))
