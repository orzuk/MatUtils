...
if(filter_by_moments)
    good_inds = internal_filter_by_moments(D, D_opt, num_moments, k_vec, n_vec, L_correction_factor, mu);
else
...
%
function good_inds = internal_filter_by_moments(D, D_opt, num_moments, k_vec, n_vec, L_correction_factor, mu)
...
region_het_moment_mat_all_models = het_moment_mat_all_models(:,1) .* 4 .* init_N .* mu; % multiply by theta (mutation rate) 
