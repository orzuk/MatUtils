...
function run_simulate_disease_pair_model(architecture_str_vec, action_mode, in_matlab_flag, ...
    ...
    if(action_mode) % run jobs
        for i=1:num_architectures % loop on architectures
            for j = 1:length(N_vec) % loop on N
                if(in_matlab_flag)
                    run_simulate_disease_pair_model(architecture_str_vec{i}, 1, 1, N_vec(j), f_vec, ...
                        disease_type_str, queue_str);
                else
                    SubmitMatlabJobToFarm(['run_simulate_disease_pair_model(''' architecture_str_vec{i} ...
