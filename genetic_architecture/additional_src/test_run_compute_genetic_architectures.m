...
architecture_str_vec = {'and-of-sigmoids'} % {'and-of-k_or_more_of_n'}; 

% {'and-of-sigmoids'}; % , 'DNF', 'CNF', 'or-of-sigmoids'}; %  'sigmoid' ... % }; % 'and-of-sigmoids',
%    'or-of-sigmoids', 'sum-of-ors', 'sum-of-ands'}; % , 'DNF',  }; % , ...
%       'sigmoid'  ,'random', 'monotone', 'and', 'xor', 'or', 'additive'}; % , 'monotone'};

action_mode = 1; in_matlab_flag = 1;
...
f_vec = [0.2]; % try one 'till bugs are fixed [0.01 0.05 0.1 0.2 0.5]; % try different f_vecs
...
for i=1:1%4 % 2:length(disease_type_str)
    [good_architectures good_architectures_file ...
        good_architectures_plot_file good_architectures_latex_file] = ...
        run_compute_genetic_architectures(architecture_str_vec, ...
