...
if(~isfield(trait_struct, 'type', 'var'))
...
if(isscalar(D)) % equilibrium model
...
switch compute_flag
    ...
    case 'simulation'
        [x_vec, neutral_allele_freq_hist, ~, ~, demographic_compute_time] = ...
...
T_0 = sum(neutral_allele_freq_hist) * (x_vec(2)-x_vec(1)); %T_0 = absorption_time_by_selection(0, 1, N, 1/(2*N), 1-1/(2*N), 0); % use analytic approximation (not histogtam). Turns out to matter a lot! %%% T_0 = integral_hist(x_vec, neutral_allele_freq_hist); %%% T_0 = absorption_time_by_selection(0, 1, N, 1/(2*N), 1-1/(2*N), 0); % 'freq');
...
for i_s = 1:num_s % loop on parameters
    ...
    switch compute_flag
        ...
        case 'simulations'
            [~, null_allele_freq_hist, ~, ~, demographic_compute_time] = ...
                compute_allele_freq_spectrum_from_demographic_model(D, s, compute_flag); % Try a grid of different values
    end
    sum_null_allele_freq_hist = sum(null_allele_freq_hist);
    T_s(i_s) = sum(null_allele_freq_hist) * (x_vec(2)-x_vec(1));     %%    T_s(i_s) = absorption_time_by_selection(-s_null_vec(i_s), 1, N, 1/(2*N), 1-1/(2*N), 0); % use analytic approximation (not histogtam). Turns out to matter a lot!     %    T_s = integral_hist(x_vec, null_allele_freq_hist); %%% T_s = absorption_time_by_selection(-s_null_vec(i_s), 1, N, 1/(2*N), 1-1/(2*N), 0); % 'freq'); % sure we need to use 'freq' here ???
    ...
    if(poisson_model_flag) % compute poisson part of likelihood
        ...
        %        tmp_z0_vec = exp( num_individuals_vec(1) .* (log_one_minus_x_vec)); %  - log(underflow_correction_q(1))  ); % what's this? multinomial/binomial coefficient?
        prob_null_allele_polymorphic_in_population(i_s) = 4 * N * D.mu * T_s(i_s); % prob. allele polymorphic in population. Is this stuff just for equilibrium? 
    ...
    for i_alpha = 1:num_alpha
        ...
        if(poisson_model_flag) % compute poisson part of likelihood  (observation of polymorphic alleles)
            log_like_mat(i_s, i_alpha, :) = internal_log_like(target_size_null_alleles, target_size_neutral_alleles, target_size_missense_alleles, ...
                num_polymorphic_null_alleles_observed, num_polymorphic_neutral_alleles_observed, num_polymorphic_missense_alleles_observed, ...
                prob_null_allele_polymorphic_in_sample, prob_neutral_allele_polymorphic_in_sample, prob_missense_allele_polymorphic_in_sample, ...
...
%                log_like_mat(i_s,i_alpha,i_beta) = log_like_mat(i_s,i_alpha,i_beta) + 0 * sum(log_like_correction); % add correction. Just a constant (who cares)
            ...
            if(include_phenotype) % compute phenotypes contribution to likelihood
                ...
                log_like_mat(i_s,i_alpha,i_beta) = internal_log_like_pheotype(log_like_mat(i_s,i_alpha,i_beta), ...
                    X, y, alpha_vec(i_alpha), beta_vec(i_beta), num_individuals, num_alleles_vec, optimize_alpha, prob_null_given_x)
                ...
                if(optimize_alpha)       % Perform exponential search on all values of w (which alleles are null and which not). Is this the right formula?
                    for i=1:n
                        ...
                        cond_y_tab(i,1:num_alleles_vec(i)+1) = internal_phenotype_fun( ...
                            y(i), beta_vec(i_beta).*(0:num_alleles_vec(i)), sigma_e, trait_struct);
                        %                        end
                    end
