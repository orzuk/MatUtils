% Simulate a disease model with pairwise (and higher-order ?) interactions
% What the hell should this function do?
%
% Input:
...
% lods_ratio_marginal - ratio of disease prob. between someone with and without risk genotype
% p_z_x_marginal
...
function [candidate_architectures mu V v_marginal v_environment v_genetic V_helper ...
    ...
    h h_add h_liability ... 
    mz_twin_risk lods_ratio_marginal p_z_x_marginal lods_ratio_pairwise mu_pairwise ...
    mu_given_k_ones mu_given_k_ones_std relative_risk family_tree family_risk ...
    architecture_formula full_calc_flag] = ...
    simulate_disease_pair_model(f_vec, architecture_str, params_struct, ...
...
v_marginal = zeros(N,iters); lods_ratio_marginal = zeros(N,iters);
...
if(~isempty(full_calc_inds))
    ...
    [v_marginal(:,full_calc_inds) lods_ratio_marginal(:,full_calc_inds) ...
    ...
    full_calc_flag = full_calc_flag & ((r >= ratio_interval(1)) & ...
        (r <= ratio_interval(2)));
    full_calc_flag = full_calc_flag & ((max(lods_ratio_marginal) >= lods_interval(1)) & ...
        (max(lods_ratio_marginal) <= lods_interval(2)));
end
...
if(~isempty(full_calc_inds))
    ...
    [mu_given_k_ones(:,:,full_calc_inds) mu_given_k_ones_std(:,:,full_calc_inds) ...
        ...
        h_liability(full_calc_inds)] = ...
        compute_architecture_statistics_extra(architecture_str, ...
        f_vec, params_struct, sampling_iters, ...
        mu(full_calc_inds), V(full_calc_inds), v_environment(full_calc_inds), ... % add architecture parameters as input
