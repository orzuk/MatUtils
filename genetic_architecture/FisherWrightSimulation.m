...
switch compute_mode % add simulations details 
    case 'simulation'
        simulation_struct = var2struct(q, num_simulated_polymorphic_alleles_vec, frac_polymorphic_vec, prob_fixation, frac_old_alleles_survived_vec, frac_het_kept_vec, ...
            prob_site_polymorphic_at_equilibrium, num_simulated_polymorphic_alleles_vec); % add many things here 
    otherwise
        simulation_struct = [];
end
...
function [x_vec, p_vec, total_het_at_each_generation_vec, num_absorptions, num_fixations, num_absorptions_by_generation_vec, count_vec, ...
    ...
    compute_numeric_expansion_internal(N, s, mu, two_side_flag, num_generations, N_vec, max_N, init_str, compute_matrix)
...
[q, weights, total_het_at_each_generation_vec, ...
    num_absorptions, num_fixations, num_losses, ...
    num_absorptions_by_generation_vec, num_fixations_by_generation_vec, num_losses_by_generation_vec, ...
    absorption_time_given_init_freq_vec, fixation_time_given_init_freq_vec, loss_time_given_init_freq_vec, ...
    num_simulated_polymorphic_alleles_vec, count_vec] = deal([]);


x_vec = cell(num_generations, 1);  
...
if(use_moments) % this works only for s=0 !!!!
    ...
    for j=1:num_generations
        x_vec{j} = (1:(2*N-1)) ./ (2*N);
        [mu_vec_expansion_analytic, mu_vec_equilibrium] = FisherWright_Compute_SFS_Moments(N_vec(1:j), 0, max_k); % compute moments with Formulas from Ewens
