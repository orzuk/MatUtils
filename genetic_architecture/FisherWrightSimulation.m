...
if(~exist('init_str', 'var') || isempty(init_str))
    init_str = 'equilibrium'; % 'newly_born'; % start at newly born allele or equilibrium
end
...
% % %         absorption_time_given_init_freq_vec(:) = num_generations * iters / (num_losses + num_fixations); % get both types of absorptions (what's here???)  
    ...
    case 'numeric' % here compute everything by matrix multiplications
        [x_vec p_vec total_het_at_each_generation_vec num_absorptions num_fixations num_absorptions_by_generation_vec count_vec ...
    ...
    case 'analytic' % compte solution based on Jacobi polynomials (See e.g. Kryukov et al. PNAS 2009).
        [x_vec p_vec total_het_at_each_generation_vec num_absorptions num_fixations num_absorptions_by_generation_vec count_vec ...
...
if(plot_flag) % Plot individual trajectories
    plot_expansion_internal(N, mu, s, q, p_vec, p_vec_equlibrium_analytic);
end % if plot


freq_struct = var2struct(x_vec, p_vec, p_vec_equlibrium_analytic, het_vec, total_het_at_each_generation_vec, ...
...
rand_str = 'poisson'; % 'binomial'; % How to simulate: poisson is much faster (approximation)
...
while( (num_alleles_simulated < iters) && (num_simulated_polymorphic_alleles_vec(1) < 20000) ) % simulate blocks. Problem: No new alleles born here! (these can be simulated separatey?)
    ...
    q = zeros(block_size, num_generations, 'single'); % , 'single'); % matrix of derived allele frequencies at each generation
    weights = ones(block_size, num_generations, 'single'); % , 'single'); % weigh later generations with higher weights (why?)
...
% Output:
%
function [total_het_at_each_generation_vec num_absorptions num_fixations num_absorptions_by_generation_vec count_vec ...
    absorption_time_given_init_freq_vec fixation_time_given_init_freq_vec num_losses loss_time_given_init_freq_vec num_simulated_polymorphic_alleles_vec] = ...
    compute_analytic_expansion_internal( ...
    N, s, mu, two_side_flag, num_generations, N_vec, max_N, init_str)
...
%
function plot_expansion_internal(N, mu, s, q, p_vec, p_vec_equlibrium_analytic)
...
title(['Density of alleles for each allele frequency. ' params_str ...
    ' (' num2str(num_generations) ' generations, ' num2str(iters) ' iterations, ' num2str(num_effective_iters) ' effective iters)']);
