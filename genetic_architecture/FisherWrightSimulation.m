...
switch compute_mode % add simulations details
    case 'simulation'
        ...
        simulation_struct = var2struct(q, num_simulated_polymorphic_alleles_vec, ...
            ...
            prob_site_polymorphic_at_equilibrium, prob_site_polymorphic_at_end); % add many things here
...
switch init_str
    ...
    case 'equilibrium'
        ...
        init_p_vec = init_p_vec .* 2.* max(10^(-10), mu); % 4.*N.*mu; % multiply by theta. (If mu=0: no mutations, then we've got a problem ... set some mu>0)
...
for j=1:num_generations % loop on # of generations
    ...
    prob_compute_method = 'exact'; % 'approximate'; % 'exact'; 
...
if(use_moments) % this works only for s=0 !!!!
    ...
    for j=1:num_generations
        ...
        [lambda_max_ent, g_het_max_ent, entr_max_ent] = ...  % Fit density using moments % , lambda0)
            me_dens2(mu_vec_expansion_analytic(2:end) ./ mu_vec_expansion_analytic(1), x_vec{j}, [], 0); % normalize by 0th moment. Don't plot anything
        ...
        p_vec{j} = [0 f_max_ent 0]; % get p. Add zeros ad boundaries 
