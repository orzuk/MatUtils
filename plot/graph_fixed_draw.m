% Like graph draw, but with fixed layout given as input
%
% The input:
% adj - adjancety matrix
% x - layout x coordinates
% y - layout y coordinates
% labels - label for each node
%
% The output:
% h - handle for graphviz (?)
%
function [h] = graph_fixed_draw(adj, x, y, labels)
...
%      'X'  Coordinates of nodes on the unit square <Default : calls
%      make_layout>
%      'Y'
%
...
for i = 1:N,
    ...
    wy = sz(4);
    wx = max(2/3*sz(3), wy);
    wx = 0.4* wx;        %  might want to play with this .9 and .5 coefficients
    wy = 0.4 * wy;
