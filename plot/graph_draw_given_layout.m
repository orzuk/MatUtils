% Like graph draw but when layout is given as input 
% 
% Input: 
% adj - adjancency matrix
% XY - a vector of size 2xn with (X,Y) coordinates
% 
function [x, y, h] = graph_draw_given_layout(adj, XY)
...
%      'node_weights' - new! allow also node weights 
...
% New: check if adj contains also weights information.
if(~isempty(intersect(find(adj ~= 0) , find(adj ~= 1)))) 
    weights = adj; % save the weights
    adj(adj ~= 0) = 1; % convert to 'standard' graph
...
x = XY(1,:); y = XY(2,:); % get x and y layout 

idx1 = find(node_t == 0); wd1 = [];   %  Draw oval nodes 
if ~isempty(idx1),
    [h1 wd1] = textoval(x(idx1), y(idx1), labels(idx1), fontsize, color(idx1,:));
end;
idx2 = find(node_t ~= 0); wd2 = [];   %  Draw square nodes 
if ~isempty(idx2),
    [h2 wd2] = textbox(x(idx2), y(idx2), labels(idx2), color(idx2,:));
end;
    ...
    if  (adj(node2,node) == 0)           % if directed edge
        ...
        if(exist('weights', 'var'))
            line([x(node)+dx1 x(node2)-dx2], [y(node)+dy1 y(node2)-dy2], ...
                'Color', color_mat(   max(1,ceil( ncolors * (weights(node,node2) - min_w) / range_w  )),: ), ...
                'LineStyle', linestyle, 'LineWidth', linewidth * weights(node,node2) / min_w  );
        else
            line([x(node)+dx1 x(node2)-dx2], [y(node)+dy1 y(node2)-dy2], ...
...
if(exist('weights', 'var'))
    colorbar('YTickLabel', [min_w:range_w/10:max_w]);
end
