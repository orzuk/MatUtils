...
% $Revision: 1.1.6.12 $  $Date: 2010/01/25 21:42:02 $
% Edited by Tal Shay to allow for a figure per slide
if ~ispc
...
% % CHANGE (TS) Get slide height and width
scrsz = get(0,'ScreenSize');
slideHeight = scrsz(4);
slideWidth = scrsz(3);
openPresentation.PageSetup.SlideHeight = slideHeight;
openPresentation.PageSetup.SlideWidth = slideWidth;

% % end CHANGE (TS) instead of following
...
%imgWidth = 350;
imgWidth = fullWidth - 2*margin - 1; % My change (TS)
...
for i = 1:cellNodeList.getLength
   ...
   while ~isempty(childNode)
      if isequal(char(childNode.getNodeName),'img')
        ...
        % My change (TS) 
        left = (slideWidth-imgWidth-margin); 
        top = margin; % My change (TS)
%          % end My change (TS) instead of
...
%          top = margin + 10*imgCount;
         ...
         try
             ...
             if strcmp(e.identifier,'MATLAB:imread:fileFormat')
                 vectorFormat = true;
             else
                 throw(e)
             end
         ...
         % my change (TS) to put each image on a separate slide
          slide = addSlide(openPresentation,presentationSlides);
          % end changes (TS)addSlide
...
for k = 1:pChildNodeList.getLength
   ...
   elseif strcmp(char(pChildNode.getFirstChild.getNodeName),'img')
      ...
      nextText = char(pChildNode.getFirstChild.getTextContent); 
   end
