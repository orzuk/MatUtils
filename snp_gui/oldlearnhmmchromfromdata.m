...
function [HMM_MODEL_SAVED, Viterbi_Path, Gamma_Probs, HMM_CHROM_KL_DIST, PatChromMeanMatrix, PatChromStdMatrix] = ...
    ...
    learn_model_EM, find_path_Viterbi, compute_chromosomes_distance_matrix, use_locations, do_center_norm, do_fold_change, do_determine_x_dim, do_median_flag, ...
...
% We always load the needed data
...
if(DataType == SNP_CHIPS) % Work for now only on the Hind SNP chip
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Take all datas in order to find the damn  SNP Frequencies - DEBUGGING ONLY!!!
    ALL_DATA = ...
        2*(HD78_9_diag_calls_hind == 1) + (HD78_9_diag_calls_hind == 2) + ...
        2*(HD78_9_rem_calls_hind == 1) + (HD78_9_rem_calls_hind == 2) + ...
        2*(HD82_3_diag_calls_hind == 1) + (HD82_3_diag_calls_hind == 2) + ...
        2*(HD82_3_rem_calls_hind == 1) + (HD82_3_rem_calls_hind == 2) + ...
        2*(HD86_7_diag_calls_hind == 1) + (HD86_7_diag_calls_hind == 2) + ...
        2*(HD86_7_rem_calls_hind == 1) + (HD86_7_rem_calls_hind == 2) + ...
        2*(HD90_1_diag_calls_hind == 1) + (HD90_1_diag_calls_hind == 2) + ...
        2*(HD90_1_rem_calls_hind == 1) + (HD90_1_rem_calls_hind == 2) + ...
        2*(HD92_3_diag_calls_hind == 1) + (HD92_3_diag_calls_hind == 2) + ...
        2*(HD92_3_rem_calls_hind == 1) + (HD92_3_rem_calls_hind == 2) + ...
        2*(TEL74_5_diag_calls_hind == 1) + (TEL74_5_diag_calls_hind == 2) + ...
        2*(TEL74_5_rem_calls_hind == 1) + (TEL74_5_rem_calls_hind == 2) + ...
        2*(TEL76_7_diag_calls_hind == 1) + (TEL76_7_diag_calls_hind == 2) + ...
        2*(TEL76_7_rem_calls_hind == 1) + (TEL76_7_rem_calls_hind == 2) + ...
        2*(TEL80_1_diag_calls_hind == 1) + (TEL80_1_diag_calls_hind == 2) + ...
        2*(TEL80_1_rem_calls_hind == 1) + (TEL80_1_rem_calls_hind == 2) + ...
        2*(TEL84_5_diag_calls_hind == 1) + (TEL84_5_diag_calls_hind == 2) + ...
        2*(TEL84_5_rem_calls_hind == 1) + (TEL84_5_rem_calls_hind == 2) + ...
        2*(TEL88_9_diag_calls_hind == 1) + (TEL88_9_diag_calls_hind == 2) + ...
        2*(TEL88_9_rem_calls_hind == 1) + (TEL88_9_rem_calls_hind == 2) + ...
        2*(TEL94_5_diag_calls_hind == 1) + (TEL94_5_diag_calls_hind == 2) + ...
        2*(TEL94_5_rem_calls_hind == 1) + (TEL94_5_rem_calls_hind == 2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
...
for i=ChromosomesToRun %%% num_chroms % go over all chromosomes
    ...
    [JointSNPs III JJJ] = intersect(LD{i}.Hind.Names, rs_ids_hind(indexes_vec)); % need to do further intersection since some SNPs
    indexes_vec = indexes_vec(JJJ);
    MissInds = setdiff( [1:length(LD{i}.Hind.Names)], III );

    % take the sub-matrix of genes on this chromsome
    HMM_chrom_data_mat_copy{i} = zeros(length(LD{i}.Hind.Names),1);
    HMM_chrom_data_mat_copy2{i} = zeros(length(LD{i}.Hind.Names),1);
    HMM_chrom_loc_mat_copy{i} = zeros(length(LD{i}.Hind.Names),1);
    ...
    for j=MissInds
        HMM_chrom_data_mat_copy{i}(j) = ...
            (HMM_chrom_data_mat_copy{i}(max(j-1,1)) + HMM_chrom_data_mat_copy{i}(min(j+1,length(LD{i}.Hind.Names))))/2;
        HMM_chrom_data_mat_copy2{i}(j) = ...
            (HMM_chrom_data_mat_copy2{i}(max(j-1,1)) + HMM_chrom_data_mat_copy2{i}(min(j+1,length(LD{i}.Hind.Names))))/2;
        HMM_chrom_loc_mat_copy{i}(j) = ...
            (HMM_chrom_loc_mat_copy{i}(max(j-1,1)) + HMM_chrom_loc_mat_copy{i}(min(j+1,length(LD{i}.Hind.Names))))/2;
    end
...
for i=ChromosomesToRun %%% num_chroms % go over all chromosomes
    ...
    num_genes_in_chrom(i) = length(LD{i}.Hind.Names);
    ...
    if(copy_flag == 1)
        if(DataType == SNP_CHIPS) % Fill all the needed details (without learning ... )
            ...
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M = zeros(4,length(LD{i}.Hind.PairMat{1}));
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M(1,:) = (LD{i}.Hind.PairMat{1}+derich) ./ (LD{i}.Hind.PairMat{1}+LD{i}.Hind.PairMat{2}+2*derich);
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M(3,:) = 1-HMM_MODEL_SAVED{i}{K_min}.PLACE_M(1,:);  % Switch 2 and 3 for the C function
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M(2,:) = (LD{i}.Hind.PairMat{3}+derich) ./ (LD{i}.Hind.PairMat{3}+LD{i}.Hind.PairMat{4}+2*derich);
            ...
            HT = hind_table(I+1,:);
            HT_SIGNS = zeros(1,size(HT,1));
            HT_SIGNS(strmatch('+', HT(:,13))) = 1; % '+' strand is 1
            CHR_HT_SIGNS = zeros(1,length(LD{i}.Hind.FreqVec));
            CHR_HT_SIGNS(III) = HT_SIGNS(indexes_vec);
            %%% LD_corrected_FreqVec = 2 .* LD{i}.Hind.FreqVec .* CHR_HT_SIGNS' - LD{4}.Hind.FreqVec - CHR_HT_SIGNS' + 1;

            HMM_MODEL_SAVED{i}{K_min}.PLACE_M2 = zeros(4,length(LD{i}.Hind.PairMat{1}));
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M2(1,:) = (LD{i}.Hind.FreqVec(2:end)+derich) ./ (1+2*derich);
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M2(3,:) = 1-HMM_MODEL_SAVED{i}{K_min}.PLACE_M2(1,:);  % Switch 2 and 3 for the C function
            HMM_MODEL_SAVED{i}{K_min}.PLACE_M2(2,:) = (LD{i}.Hind.FreqVec(2:end)+derich) ./ (1+2*derich);;
