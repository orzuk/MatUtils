% The function computes the log-likelihood of a sequence according to 
% a given HMM. There HMP can be 'standard' or SNP-based, and also the
% sequence can contain the states (X) in which case the computation is
% trivial, or only the observation, (Y state), in which case we use the
% forward algorithm to compute the likelihood. 
% Should we do this in C for faster implementation? probably yes!!!
