...
%
function ErrorStruct = HapMapCompareAffyAndHMMGenotypes(SampleNames, chip_type, user_dir, ALL_MAT, ...
...
comp_hap_local = ~isempty(ALL_MAT);

if(comp_hap_local)
    ErrorStruct.hap_local_chr_common_snps_vec = zeros(1,22); % How many SNPs are on the chip and HAPMAP in each chrom
    ErrorStruct.hap_local_chr_error_snps_vec = zeros(nsamples,22); % For how many SNPs did we get a different genotype
    ErrorStruct.hap_local_uneq_inds_mat = zeros(nsamples, length(ALL_MAT.snp_ids));
    % Special temp
    ErrorStruct.hap_nocalls_mat = zeros(nsamples, length(ALL_MAT.snp_ids));
end
...
if(comp_hap_local)
    for chrom=chroms
        [snp_ids_intersect{chrom} III{chrom} JJJ{chrom}] = intersect(ALL_MAT.snp_ids, R.SnpsIDs{chrom}); % Get the common SNPs from disp
...
for cur_sample = 1:nsamples
    ...
    if(comp_hap_affy || comp_affy_hmm)
        eval(['Aff = load(fullfile(user_dir, [SampleNames{cur_sample} ''_'' chip_type ''.mat'']));']);  % Should we load also the allele ratios? Yes if we want to plot stuff!
    end

    % Load the results of the HMM - from the display !! 
    if(comp_hap_hmm || comp_affy_hmm)
        eval(['load(fullfile(user_dir, ''display'' , [SampleNames{cur_sample} ''_'' chip_type ''_disp'']));']); % load display file
    end
    ...
    for chrom=chroms
        ...
        if(comp_hap_local)
%%            [snp_ids_intersect I J] = intersect(ALL_MAT.snp_ids, R.SnpsIDs{chrom}); % Get the common SNPs from disp
            ...
            LOCAL_SNPs = ALL_MAT.data_genotype_AB(III{chrom},cur_sample);
        ...
        if(chrom*cur_sample == 0) %% 1) %% plot or not? 
            if(comp_hap_affy)
                PlotAlleleRatios(Aff.copy_num_vec(hap_affy_I), Aff.allele_ratio_vec(hap_affy_I), max(0,HAPMAP_SNPs), ['HAPMAP genotypes chrom ' num2str(chrom) ' Sample ' SampleNames{cur_sample}]);
                PlotAlleleRatios(Aff.copy_num_vec(hap_affy_I), Aff.allele_ratio_vec(hap_affy_I), Aff.genotype_vec(hap_affy_I), ['affy genotypes chrom ' num2str(chrom) ' Sample ' SampleNames{cur_sample}]);
                PlotAlleleRatios(Aff.copy_num_vec(hap_affy_I), Aff.allele_ratio_vec(hap_affy_I), AFFY_SNPs == HAPMAP_SNPs, ['affy vs. hap genotypes  Sample ' SampleNames{cur_sample}]);
            end

            if(comp_affy_hmm || comp_hap_affy)
                PlotAlleleRatios(Aff.copy_num_vec, Aff.allele_ratio_vec, Aff.genotype_vec, ['affy genotypes  Sample ' SampleNames{cur_sample}]);
            end
           if(comp_hap_hmm)
               PlotAlleleRatios(Aff.copy_num_vec(affy_hmm_I), Aff.allele_ratio_vec(affy_hmm_I), HMMOut_SNPs == HAPMAP_SNPs, ['hmm genotypes vs. HapMap genotypes chrom ' num2str(chrom) ' Sample ' SampleNames{cur_sample}]);
           end
            
            if(comp_affy_hmm)
                PlotAlleleRatios(Aff.copy_num_vec(affy_hmm_I), Aff.allele_ratio_vec(affy_hmm_I), max(0, AFFY_SNPs), ['affy genotypes chrom ' num2str(chrom) ' Sample ' SampleNames{cur_sample}]); % Aff.genotype_vec(affy_hmm_I)
                PlotAlleleRatios(Aff.copy_num_vec(affy_hmm_I), Aff.allele_ratio_vec(affy_hmm_I), HMMOut_SNPs, ['hmm genotypes chrom ' num2str(chrom) ' Sample ' SampleNames{cur_sample}]);
                PlotAlleleRatios(Aff.copy_num_vec(affy_hmm_I), Aff.allele_ratio_vec(affy_hmm_I), HMMOut_SNPs == AFFY_SNPs, ['hmm genotypes vs. Affy genotypes chrom ' num2str(chrom) ' Sample ' SampleNames{cur_sample}]);
            end
        end
