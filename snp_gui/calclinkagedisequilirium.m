...
%
% This function calculates the Linkage Disequilibrium (LD) values of all pairs
% of adjacent SNPs. 
%
% The inputs are:
...
% OffspringsInds - The childs we ignore in the statistics (can be empty), since we know their parents
% LD_flag - currently not used
% use_hetro_flag - whether or not to use heterozyguos from hapmap for
% statistics (since we don't know if its AB or BA for them)
% Calc_LD_mat_flag - whether or not to calculate the LD matrix
%
% The outputs are:
% LD_mat - matrix of LD parameters
% LocalFreqVec - a vector of single SNPs frequencies
% PairwiseFreqs - frequencies of pairs of SNPs
...
PairwiseFreqs = zeros(N-1,4);  % 00 01 10 11
...
if(use_hetro_flag == 1) % Use all data assuming it is paired correctly
    ...
    PairwiseFreqs = PairwiseFreqs ./ (120 - repmat(PairwiseBadCounts, 1, 4));
