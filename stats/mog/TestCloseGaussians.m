% Test how well can we reconstruct MoG parameters for two very close gaussians. 
...
mu = [0, 4]; % means
sigma = [1, 1]; % std.s 
prior = [0.1 0.9]; % probs. for each gaussian
n_points = 10^5;
data = MixtureOfGaussiansSimulateData(prior, mu, sigma, n_points); % simulate data  
...
[P,M,S, LogLike] = MixtureOfGaussiansEM(data',num_g,iters,start_points); % % Learn model using EM. Last argument is dummy


INIT_P = [0.8 0.2]; INIT_M = [2.2 0.8]; INIT_S = [0.9 1.1]; % give 'close' initial conditions
% [P2,M2,S2, LogLike2] = MixtureOfGaussiansGivenInitMatlab(data',num_g,iters, INIT_P, INIT_M, INIT_S);
[P3,M3,S3, LogLike3] = MixtureOfGaussiansGivenInit(data',num_g,iters, INIT_P, INIT_M, INIT_S);
[P4,M4,S4, LogLike4] = MixtureOfGaussiansGivenInit(data',num_g,iters, prior, mu, sigma);

[P_F M_F S_F LogLike_F] = MixtureOfGaussiansGivenInit(data',num_g,iters, prior, mu, sigma, ...
    [0 0], [1 0], [1 0]) % try learning when some variables are freezed 
...
num_bins = 100; 
labels_vec = {'MoG data', 'freq.'}; legends_vec = {'1st Gauss.', '2nd Gauss.', 'Mix. Gauss.',}
figure; [h_data h_bin_loc] = hist_density(data,num_bins); 
figure; hist(data,num_bins*5); 
MixtureOfGaussiansDraw1dGaussians(data, P, M, S, labels_vec, legends_vec, [], [], num_bins);

model_ll = MixtureOfGaussiansGetLikelihood(data, prior, mu, sigma) 
