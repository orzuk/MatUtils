% Generate HMP entropy plots for paper's figs 
...
for eps = eps_vec 
    ...
    tol=0.0002;
    p_vec = [0.24:tol:0.3-tol]; % Set the values of p we want
    ...
    ent_five_orders_est_vec = Hp_zero + eps .* Hp(1,:) + (eps^2) .* Hp(2,:) + eps^3 .* Hp(3,:) + eps^4 .* Hp(4,:) + eps^5 .* Hp(5,:) + ...
    eps^6 .* Hp(6,:) + eps^7 .* Hp(7,:) + eps^8 .* Hp(8,:) + eps^9 .* Hp(9,:); %  + eps^10 .* Hp(10,:); %  + eps^11 .* Hp(11,:); 
    
    ent_five_orders_est_vec = ent_five_orders_est_vec ./ log(2.0); % Transfer from natural to binary logarithm
    
    ent_seven_orders_est_vec = Hp_zero + eps .* Hp(1,:) + (eps^2) .* Hp(2,:) + eps^3 .* Hp(3,:) + eps^4 .* Hp(4,:) + eps^5 .* Hp(5,:) + ...
    eps^6 .* Hp(6,:) + eps^7 .* Hp(7,:); %% + eps^8 .* Hp(8,:) + eps^9 .* Hp(9,:); %  + eps^10 .* Hp(10,:); %  + eps^11 .* Hp(11,:); 
    ...
    % Now the first plot in a seperate figure 
     hold on; 
    subplot(2,1,i); hold on; plot(p_vec, ent_orders_est_vec, 'r'); plot(p_vec, ent_five_orders_est_vec, 'g');  plot(p_vec, ent_seven_orders_est_vec, 'c'); %plot([0:101], log(abs([Hp_zero Hp(1:101)'])), '+');  
    plot(p_vec, C_N, '.b'); plot(p_vec, cX_N, '-.m');
    title(['HMP Order-Based Estimated Entropy for \epsilon= ' num2str(eps)]); xlabel('p'); ylabel('Entropy Estimation'); %legend('I.I.D.', 'HMM');
    legend('11-term Expansion', '9-term Expansion', '7-term Expansion', 'Upper Bound', 'Lower Bound');                        
