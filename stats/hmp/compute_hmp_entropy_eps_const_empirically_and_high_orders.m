% Compute HMP entropy empirically and high-order approx. for fixed eps.
...
eps_vec = [0.01 0.2]; % Set the epsilon we want to work with
...
for eps = eps_vec 
    ...
    % Now the first plot in a seperate figure 
     hold on; 
    subplot(1,2,i); hold on; plot(p_vec, ent_orders_est_vec, 'r'); %plot([0:101], log(abs([Hp_zero Hp(1:101)'])), '+');  
    plot(p_vec, C_N, '.b'); plot(p_vec, cX_N, '-.m');
    title(['HMP Order-Based Estimated Entropy for epsilon= ' num2str(eps)]); xlabel('p'); ylabel('Entropy Estimation'); %legend('I.I.D.', 'HMM');
    legend('Expansion', 'Upper Bound', 'Lower Bound');                        
