% Generate HMP entropy plots for paper's fig for eps constant (opposite)
...
% This is good for p=0.2 : eps_vec = [tol:tol:0.065-tol];  % [0.1]; % Set the epsilon we want to work with

eps_vec = [tol:tol:0.495-tol];  % [0.1]; % Set the epsilon we want to work with

 p_vec = 0.45; % *ones(1, length(eps_vec));  % Set the values of p we want
...
% Do to get two epsilons ...
    ...
    C_N(ind) = HMP_entropy_finite(eps, p_vec, N)-HMP_entropy_finite(eps, p_vec, N-1); % The upper bound
    cX_N(ind) = HMP_entropy_finite_X1(eps, p_vec, N)-HMP_entropy_finite_X1(eps, p_vec, N-1); % The lower bound
    ind = ind+1;
end

    % New plot : Constant epsilon. Verious values of p : 
    ent_orders_est_vec = Hp_zero + eps_vec .* Hp(1,:) + (eps_vec.^2) .* Hp(2,:) + eps_vec.^3 .* Hp(3,:) + eps_vec.^4 .* Hp(4,:) + eps_vec.^5 .* Hp(5,:) + ...
        eps_vec.^6 .* Hp(6,:) + eps_vec.^7 .* Hp(7,:) + eps_vec.^8 .* Hp(8,:) + eps_vec.^9 .* Hp(9,:) + eps_vec.^10 .* Hp(10,:) + eps_vec.^11 .* Hp(11,:); 
    
    ent_orders_est_vec = ent_orders_est_vec ./ log(2.0); % Transfer from natural to binary logarithm
    
    
    % Here take expansions using less orders : 
    ent_five_orders_est_vec = Hp_zero + eps_vec .* Hp(1,:) + (eps_vec.^2) .* Hp(2,:) + eps_vec.^3 .* Hp(3,:) + eps_vec.^4 .* Hp(4,:) + eps_vec.^5 .* Hp(5,:) + ...
    eps_vec.^6 .* Hp(6,:) + eps_vec.^7 .* Hp(7,:) + eps_vec.^8 .* Hp(8,:) + eps_vec.^9 .* Hp(9,:)  + eps_vec.^10 .* Hp(10,:); %  + eps^11 .* Hp(11,:); 
    
    ent_five_orders_est_vec = ent_five_orders_est_vec ./ log(2.0); % Transfer from natural to binary logarithm
    
    ent_seven_orders_est_vec = Hp_zero + eps_vec .* Hp(1,:) + (eps_vec.^2) .* Hp(2,:) + eps_vec.^3 .* Hp(3,:) + eps_vec.^4 .* Hp(4,:) + eps_vec.^5 .* Hp(5,:) + ...
    eps_vec.^6 .* Hp(6,:) + eps_vec.^7 .* Hp(7,:) + eps_vec.^8 .* Hp(8,:) + eps_vec.^9 .* Hp(9,:); %  + eps^10 .* Hp(10,:); %  + eps^11 .* Hp(11,:); 
    ...
    % Now the first plot in a seperate figure 
     hold on; 
    subplot(2,1,i); hold on; plot(eps_vec, ent_orders_est_vec, 'g'); plot(eps_vec, ent_five_orders_est_vec, 'r');  plot(eps_vec, ent_seven_orders_est_vec, 'c'); %plot([0:101], log(abs([Hp_zero Hp(1:101)'])), '+');  
    plot(eps_vec, C_N, 'b'); plot(eps_vec, cX_N, '-m');
    legend('11-term Expansion', '10-term Expansion', '9-term Expansion', 'Upper Bound', 'Lower Bound');                        
    title(['High-SNR Entropy-Rate Expansion for p= ' num2str(p_vec(1))]); xlabel('\epsilon'); ylabel('Entropy-Rate'); %legend('I.I.D.', 'HMM');
