                ...
                if(~kron_flag) % estimate Sigma
                    ...
                    for j=3:-1:1 % estimate two parts using Ridge
                        if(j < 3)
                            ...
                            beta(1:r) = sqrt( (-c2(1:r) - sqrt(c2(1:r).^2 - 4.*c1.*c3(1:r)))  ./ (2.*c1) );
                            theta = repmat(2*sqrt(lambda/q), pp, 1);
                            theta(1:r) = eig_vals(1:r).^2 .* beta(1:r) ./ (pp .* beta(1:r).^2 - 4*lambda);
                            %beta(r) = 2*sqrt(lambda/pp);
                            %theta(r) = 2*sqrt(lambda/q); % Temp! Set r !!!
                        end
                        ...
                        if(j < 3)
                            ...
                            %
                            %                        % New! take Q_Ridge (not zero!!!) and fit two parts of P_Ridge
                            %                        Z_partial = Z((j-1)*p*q/2 + (1:(p*q/2)));
                            %                        % PERFORM SVD ON Z'*inv(P)*Z !!!!!!!!!!!!!
                            %                        P_Ridge0((1:(p/2)) + (p/2)*(j-1), (1:(p/2)) + (p/2)*(j-1)) = ...
                            %                            fminsearch(@(x) 1000000000*norm(x-x', 'fro') ...
                            %                            -KroneckerLogLike(Z_partial, x, Q_Ridge, lambda), V*diag(theta)*V');
                        ...
                        else % fit full model under H1
                            P_Ridge = V*diag(theta)*V';
                            Q_Ridge = U*diag(beta)*U';
                            P_known_Q = Z_mat'*inv(Q)*Z_mat ./ q; % Fit for known Q ('tree')
                            
                            %                        P2 = fminsearch(@(x) -KroneckerLogLike(Z, x, Q_Ridge, options.lambda), P_Ridge)
                            %
                            %
                            %                        norm(x^2 - (4*options.lambda/q)*eye(p) - x*(Z'*inv(prev_Q)*Z ./ q), 'fro'), P2) % use local search to improve
                            %
