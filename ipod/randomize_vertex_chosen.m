...
% rate
function [flipped_ind flip_time lambda_total] =randomize_vertex_chosen(X, lambda_vec)

N = length(X); % # of vertices
N_on = sum(X); % Initial # of infected vertices

X_off = find(X==0);
lambda_off_vec = lambda_vec(X_off); % take only the 'off' vertices

lambda_off_cumsum_vec = cumsum(lambda_off_vec); lambda_total= lambda_off_cumsum_vec(end);
