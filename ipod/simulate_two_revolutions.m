% Simulate two competing percolations. 
...
%
function [flip_t_avg t_mean t_std X X_order who_flipped] = ...
    simulate_two_revolutions(E_init, X_init_1, X_init_2, alpha, iters, lambda_1, lambda_2)
...
num_flipped = length(N_on+1:ceil(alpha*N) );  % How many flips do we have
who_flipped = zeros(iters, num_flipped); 
...
%     cross_edges_1 = E(X_off, X_on_1);    cross_edges_2 = E(X_off, X_on_2);
%     lam_tot_1 = sum(cross_edges_1);    lam_tot_2 = sum(cross_edges_2); 
    ...
    for i=N_on+1:ceil(alpha*N) % loop over each adoption event until revolution
        ...
        r = rand(1) * (lambda_total_1 + lambda_total_2);
        if(r < lambda_total_1)

            
            [flipped_ind delta_t lambda_total]= randomize_vertex_chosen(X,lambda_vec_1);
            ...
            lambda_total_1 = lambda_total_1-lambda_vec_1(flipped_ind) + length(flipped_neighbors_off) * lambda_1; % Remove the index which was lighten
            lambda_total_2=lambda_total_2-lambda_vec_2(flipped_ind);
            lambda_vec_1(flipped_neighbors_off) = lambda_vec_1(flipped_neighbors_off)+lambda_1;


            
        else
            [flipped_ind delta_t lambda_total]= randomize_vertex_chosen(X,lambda_vec_2);
            X(flipped_ind)=2; % mark vertex as 'blue'

            flipped_neighbors = find(E(flipped_ind,:));
            flipped_neighbors_off = intersect(find(X==0),  flipped_neighbors );
            
            lambda_total_2 = lambda_total_2-lambda_vec_2(flipped_ind) + length(flipped_neighbors_off) * lambda_2; % Remove the index which was lighten
            lambda_total_1=lambda_total_1-lambda_vec_1(flipped_ind);
            lambda_vec_2(flipped_neighbors_off) = lambda_vec_2(flipped_neighbors_off)+lambda_2;

            who_flipped(iter, i-N_on)=1;
